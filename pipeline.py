...
def preprocess_data(file: str,
    ...
    def process_examples(records, prefix=""):
        ...
        for i in range(total_training):
            ...
            print(
                f"\rDone serializing example ( {emb_t} - {lab_t} - {rec_t} ). Buffer {len(buffer)} - {timeit.default_timer() - starttime}",
                end="")
            ...
            if len(buffer) >= tf_record_file_size:
                ...
                print(
                    f"done write example file, size {os.stat(file_name).st_size} - {timeit.default_timer() - starttime}")
                try:
...
def download_from_minio(url: InputPath('Url'), data: OutputPath('Tarball')):
    from minio import Minio
    print(url)
    client = Minio(
        "ns-1.kind-1.dc.min.dev",
        access_key="kubeflow",
        secret_key="kubeflow123",
    )

    client.fget_object("datasets", "aclImdb_v1.tar.gz", data)


download_from_minio_op = kfp.components.create_component_from_func(
    func=download_from_minio,
    output_component_file='component-download.yaml',  # This is optional. It saves the component spec for future use.
    base_image='python:3.7',
    packages_to_install=['minio==7.1.10', 'tensorflow==2.2.3'], )
...
create_step_train = kfp.components.create_component_from_func(
    func=train,
...
def my_pipeline(url):
    download_from_minio_task = download_from_minio_op(url=url)
    print(download_from_minio_task.outputs)
    merge_csv_task_train = create_step_preprocess_data(file="train")
    # merge_csv_task_test = create_step_preprocess_data(file="test")
    train = create_step_train(train_path=merge_csv_task_train)
...
def run():
    ...
    # HOST = "https://kubeflow.kind-1.dc.min.dev"
